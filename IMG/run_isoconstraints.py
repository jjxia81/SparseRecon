...
main_file = r"D:\projects\IsoConstraints\Bin\x64\Release\PoissonRecon.exe "

out_dir = os.path.join(data_root_dir, "isoconstraints_recon")
...
for s_step in sample_steps:
    ...
    for i in range(len(coarse_files)):
        ...
        if coarse_file.split('.')[1] != "xyz":
            continue
        file_out = coarse_file.split('.')[0] + "_iso.ply" 
        file_out = os.path.join(out_sub_dir, file_out)
        coarse_file = os.path.join(coarse_dir, coarse_file)

        fine_file = coarse_file
        sys_cmd = main_file + coarse_file +" " + fine_file + " " + file_out + " false false false"
